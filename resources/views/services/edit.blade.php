        ...
        <nav class="flex text-sm text-gray-500 mb-2" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li>
                    <div class="flex items-center">
                        <i class="fas fa-chevron-right text-xs mx-2"></i>
                        <span class="text-gray-800 font-medium">Edit Layanan</span>
                    </div>
                </li>
            </ol>
        </nav>
        <h1 class="text-2xl font-bold text-gray-900">Edit Layanan: {{ $service->name }}</h1>
        <p class="text-sm text-gray-500 mt-1">Ubah informasi layanan medis atau perbarui tarifnya</p>
    </div>
    ...
    <form action="{{ route('services.update', $service->id) }}" method="POST">
        ...
        <div class="bg-white shadow-sm border border-gray-200 rounded-lg mb-6 overflow-hidden">
            <div class="bg-slate-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <span class="text-xs text-gray-500">Dibuat pada: {{ $service->created_at->format('d M Y') }}</span>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900">Kode Layanan <span class="text-red-500">*</span></label>
                        <input type="text" name="code" value="{{ old('code', $service->code) }}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 uppercase placeholder-gray-400">
                    </div>
                    
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900">Nama Layanan / Tindakan <span class="text-red-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name', $service->name) }}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 placeholder-gray-400">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="unit" value="{{ old('unit', $service->unit) }}" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                        </div>
                        <div>
