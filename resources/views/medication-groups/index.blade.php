...
@section('title', 'Master Data Golongan Obat')
...
<div class="w-full mx-auto pb-10" x-data="groupForm()">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-gray-800 font-medium">Golongan Obat</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-2xl font-bold text-gray-900">Golongan Obat & Alkes</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola data golongan acuan obat-obatan dan alat kesehatan</p>
        </div>
    ...
    <div class="bg-white p-4 rounded-t-lg border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
        <form action="{{ route('medication-groups.index') }}" method="GET" class="w-full md:w-96 flex">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-32">Kode Golongan</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($groups as $index => $g)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-gray-900 font-semibold">{{ $g->name }}</td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold leading-none text-white bg-blue-600 rounded-full">{{ $g->medications_count }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($g->is_active)
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center space-x-3">
                                    ...
                                    <button @click="confirmDelete('{{ route('medication-groups.destroy', $g->id) }}', '{{ $g->name }}')" class="text-red-600 hover:text-red-900 transition-colors" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-10 text-center text-gray-500">
                                ...
                                <p>Tidak ada data golongan obat.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="relative p-4 w-full max-w-lg h-full md:h-auto"
            ...
            <div class="relative bg-white rounded-xl shadow-xl overflow-hidden">
                <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Golongan Obat' : 'Tambah Golongan Obat'"></h3>
                ...
                <form :action="formAction" method="POST" class="p-6">
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Kode Golongan <span class="text-red-500">*</span></label>
                            <input type="text" name="code" x-model="formData.code" required class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                            <p class="mt-1 text-xs text-gray-500">Contoh: 8, 25.2, 30.</p>
                        </div>
                        
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Nama Golongan <span class="text-red-500">*</span></label>
                    ...
                    <div class="flex justify-end gap-3 mt-8 pt-4 border-t border-gray-200">
                        ...
                        <button type="submit" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center transition-colors" x-text="isEdit ? 'Simpan Perubahan' : 'Tambah Golongan'"></button>
                    </div>
                </form>
            </div>
        </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('groupForm', () => ({
        modalOpen: false,
        isEdit: false,
        formAction: '{{ route('medication-groups.store') }}',
        formData: {
            code: '',
            name: '',
            is_active: 1
        ...
        openModal(mode, data = null) {
            ...
            if (this.isEdit && data) {
                this.formAction = `/medication-groups/${data.id}`;
                this.formData = {
                    code: data.code || '',
                    name: data.name,
                ...
                this.formAction = '{{ route('medication-groups.store') }}';
                this.formData = {
                    code: '',
                    name: '',
                    is_active: 1
