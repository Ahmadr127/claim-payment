...
@section('title', 'Tarif Layanan Medis')
...
<div class="w-full mx-auto pb-10">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-gray-800 font-medium">Tarif Layanan Medis</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-2xl font-bold text-gray-900">Tarif Layanan Medis</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola data tarif penjualan tindakan atau layanan medis per kelas kamar</p>
        </div>
    </div>
    ...
    <div class="bg-white p-4 rounded-t-lg border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
        <form action="{{ route('service-prices.index') }}" method="GET" class="w-full flex flex-col md:flex-row gap-4">
            ...
            <a href="{{ route('service-prices.index') }}" class="bg-white hover:bg-gray-50 text-gray-500 px-4 py-2 rounded-lg border border-gray-300 transition-colors text-center">Reset</a>
        </form>
    </div>
                    ...
                    @forelse($services as $index => $srv)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-xs text-gray-500">
                                <div class="font-medium text-gray-700">{{ $srv->creator->name ?? 'System' }}</div>
                                <div class="text-[10px] text-gray-400 mt-0.5">{{ $srv->created_at ? $srv->created_at->format('d/m/Y H:i') : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-xs text-gray-500">
                                @if($srv->updated_at && $srv->updated_at != $srv->created_at)
                                    <div class="font-medium text-gray-700">{{ $srv->editor->name ?? 'System' }}</div>
                                    <div class="text-[10px] text-gray-400 mt-0.5">{{ $srv->updated_at->format('d/m/Y H:i') }}</div>
                                @else
                                    <span class="text-gray-400 italic">-</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                <a href="{{ route('service-prices.edit', $srv->id) }}" class="text-blue-600 hover:text-blue-900 font-medium inline-flex items-center gap-1.5 transition-colors" title="Kelola Tarif">
