...
@section('title', 'Master Data Komoditi Obat')

@section('content')
<div class="w-full mx-auto pb-10" x-data="commodityForm()">
    <!-- Header -->
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="text-gray-800 font-medium">Komoditi Obat</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-2xl font-bold text-gray-900">Komoditi Obat & Alkes</h1>
            <p class="text-sm text-gray-500 mt-1">Kelola data komoditi obat-obatan dan alat kesehatan</p>
        </div>
    ...
    <div class="bg-white p-4 rounded-t-lg border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
        <form action="{{ route('medication-commodities.index') }}" method="GET" class="w-full md:w-96 flex">
            <div class="relative w-full">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <i class="fas fa-search text-gray-400"></i>
                </div>
                <input type="text" name="q" value="{{ $search }}" placeholder="Cari nama komoditi..." 
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Komoditi</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($commodities as $index => $c)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4 text-gray-500">{{ $commodities->firstItem() + $index }}</td>
                            <td class="px-6 py-4 text-gray-900 font-semibold">{{ $c->name }}</td>
                            <td class="px-6 py-4 text-center">
                                <span class="inline-flex items-center justify-center px-2.5 py-1 text-xs font-bold leading-none text-white bg-blue-600 rounded-full">{{ $c->medications_count }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if($c->is_active)
                            ...
                            <td class="px-6 py-4 text-xs text-gray-500">
                                <div class="font-medium text-gray-700">{{ $c->creator->name ?? 'System' }}</div>
                                <div class="text-[10px] text-gray-400 mt-0.5">{{ $c->created_at ? $c->created_at->format('d/m/Y H:i') : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-xs text-gray-500">
                                @if($c->updated_at && $c->updated_at != $c->created_at)
                                    <div class="font-medium text-gray-700">{{ $c->editor->name ?? 'System' }}</div>
                                    <div class="text-[10px] text-gray-400 mt-0.5">{{ $c->updated_at->format('d/m/Y H:i') }}</div>
                                @else
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center space-x-3">
                                    <button @click='openModal("edit", @json($c))' class="text-blue-600 hover:text-blue-900 transition-colors" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button @click="confirmDelete('{{ route('medication-commodities.destroy', $c->id) }}', '{{ $c->name }}')" class="text-red-600 hover:text-red-900 transition-colors" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-gray-500">
                                ...
                                <p>Tidak ada data komoditi obat.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="relative p-4 w-full max-w-lg h-full md:h-auto"
            ...
            <div class="relative bg-white rounded-xl shadow-xl overflow-hidden">
                <div class="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <h3 class="text-lg font-bold text-gray-900" x-text="isEdit ? 'Edit Komoditi Obat' : 'Tambah Komoditi Obat'"></h3>
                ...
                <form :action="formAction" method="POST" class="p-6">
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block mb-2 text-sm font-medium text-gray-900">Nama Komoditi <span class="text-red-500">*</span></label>
                            ...
                            <p class="mt-1 text-xs text-gray-500">Contoh: Apotik/Obat Injeksi, Cairan Infus, Alkes.</p>
                        </div>
                    ...
                    <div class="flex justify-end gap-3 mt-8 pt-4 border-t border-gray-200">
                        ...
                        <button type="submit" class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center transition-colors" x-text="isEdit ? 'Simpan Perubahan' : 'Tambah Komoditi'"></button>
                    </div>
                </form>
            </div>
        </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('commodityForm', () => ({
        modalOpen: false,
        isEdit: false,
        formAction: '{{ route('medication-commodities.store') }}',
        formData: {
            name: '',
            is_active: 1
        },
        ...
        openModal(mode, data = null) {
            ...
            if (this.isEdit && data) {
                this.formAction = `/medication-commodities/${data.id}`;
                this.formData = {
                    name: data.name,
                    is_active: data.is_active ? 1 : 0
                };
            } else {
                this.formAction = '{{ route('medication-commodities.store') }}';
                this.formData = {
                    name: '',
                    is_active: 1
                };
        ...
        confirmDelete(url, name) {
            if(confirm(`Apakah Anda yakin ingin menghapus komoditi "${name}"?`)) {
